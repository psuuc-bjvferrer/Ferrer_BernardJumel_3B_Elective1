...
<div class="bg-white p-6 rounded-lg shadow max-w-2xl mx-auto">
    ...
    <form action="{{ route('assets.update', $asset) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" class="w-full p-2 border rounded">{{ old('description', $asset->description) }}</textarea>
        </div>

        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
            Update Asset
        </button>
        <a href="{{ route('assets.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded ml-2">Cancel</a>
    </form>
</div>
@endsection